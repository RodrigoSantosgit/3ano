...
def handle_client_connection(client_socket,address): 
    ...
    try:
        while True:
            request = client_socket.recv(1024)                      # receber mensagem do cliente
            if not request:
                client_socket.close()
            else:
                msg=request.decode()
                ...
                print()
                msg=("ECHO: "+msg).encode()                         
                client_socket.send(msg)                             # enviar mensagem de ECHO response para o cliente
    except (socket.timeout, socket.error):
        print('Client {} error. Done!'.format(address))

ip_addr = "0.0.0.0"
tcp_port = 5005                                                     # porto de escuta do servidor
